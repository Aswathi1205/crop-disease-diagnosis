...
def verify_setup():
    ...
    if not config_path.exists():
        print("  [X] model_config.json not found!")
        return False
    print(f"  [OK] model_config.json found")
    
    if not class_names_path.exists():
        print("  [X] class_names.json not found!")
        return False
    print(f"  [OK] class_names.json found")
    ...
    print(f"  [OK] Classes: {', '.join(class_names[:3])}...")
    ...
    if not test_images_dir.exists():
        print(f"  [X] Test images directory not found")
        return False
    ...
    if len(image_files) == 0:
        print("  [X] No test images found!")
        return False
    
    # Try loading a test image
    try:
        test_image = Image.open(image_files[0])
        print(f"  [OK] Successfully loaded test image: {image_files[0].name}")
        print(f"  [OK] Image size: {test_image.size}")
    except Exception as e:
        print(f"  [X] Failed to load test image: {e}")
        return False
    ...
    try:
        ...
        print(f"  [OK] Image transformed successfully")
        print(f"  [OK] Transformed shape: {transformed.shape}")
        print(f"  [OK] Transformed dtype: {transformed.dtype}")
        print(f"  [OK] Value range: [{transformed.min():.3f}, {transformed.max():.3f}]")
    except Exception as e:
    ...
    try:
        ...
        print(f"  [OK] Confidence: {confidence.item() * 100:.2f}%")
        print(f"  [OK] Output sum: {output.sum().item():.6f} (should be ~1.0)")
